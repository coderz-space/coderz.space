...
export default function CompletedQuestionsPage() {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      ...
      setQuestions(allQuestions.filter((question) => question.status === "completed"));
    };
  ...
  const handleProgressChange = async (questionId: string, progressStatus: QuestionProgressStatus) => {
    ...
    setQuestions(refreshedQuestions.filter((question) => question.status === "completed"));
  };

  return (
    <div className="max-w-3xl">
      <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Completed Questions</h1>

      {!questions.length ? (
        <p className="text-sm text-gray-500 dark:text-gray-400">No completed questions yet. Keep going.</p>
      ) : null}
        ...
        {questions.map((question) => (
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <button
                type="button"
                onClick={() => router.push(`/mentee-dashboard/${username}/completed/${question.id}`)}
                className="rounded-lg bg-purple-600 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-purple-500"
              >
                Detail
              </button>
